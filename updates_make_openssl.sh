...
do_chroot() {
	# Add qemu emulation
        cp /usr/bin/qemu-arm-static "/home/orangepi/OrangePi3G-iot/output/rootfs/usr/bin"
	...
	cp ./updates/openssl-1.1.1v-compiled_arm.tar.gz ./output/rootfs/usr/local/src
	cd ./output/rootfs/usr/local/src
	tar -xvf openssl-1.1.1v-compiled_arm.tar.gz
